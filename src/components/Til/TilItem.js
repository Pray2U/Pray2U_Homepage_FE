...
const TilItem = ({ tilInfo, onRemove, myInfo }) => {
  ...
  return (
    <div
      className={
        isClosed
          ? "relative m-auto my-[2rem] w-full h-[25rem] shadow-[1px_1px_3px_1px_#dadce0] rounded-[1em] pt-[1%] bg-[white]"
          : "relative m-auto my-[2rem] w-full h-auto shadow-[1px_1px_3px_1px_#dadce0] rounded-[1em] pt-[1%] bg-[white]"
      }
    >
      <div className="flex items-center w-[95%] h-[5rem] m-auto border-b-[0.1rem] border-b-solid border-b-[#c7c9ce]">
        ...
        />
        <div className="flex items-center justify-center h-full ml-[1rem] font-bold text-[1.5vw]">
          {tilInfo?.title}
        </div>
        {tilInfo?.userId === myInfo?.user?.writerId ? (
          <div className="flex items-center ml-auto w-[10%] h-full">
            <div
              className="flex justify-center items-center w-[2.5rem] h-[60%] bg-[#E2E2E2] rounded-[0.5rem] cursor-pointer"
              onClick={() => onEditTIL(tilInfo?.tilId)}
            >
              <AiTwotoneEdit />
            </div>
            <div
              className="flex items-center justify-center w-[2.5rem] h-[60%] ml-[0.5rem] bg-[#FFB7B7] rounded-[0.5rem] cursor-pointer"
      ...
      <div
        className={
          isClosed
            ? "w-[85%] h-[16rem] overflow-hidden m-auto pt-[1rem]"
            : "w-[85%] h-auto overflow-hidden m-auto pt-[1rem] pb-[5%]"
        }
      >
        {/* <Viewer
