    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setComments((comments) =>
          comments.filter((comm) => comm.replyId !== replyId)
        );
      } else {
  ...
  return (
    <>
      <div className="CommentFormContainer">
        <div className="CommentMyInfo">
          <img
            ...
            alt="profile"
          />
          <div className="CommentWriter font-nanumgothic font-semibold">
            {myInfo?.username}
          </div>
        </div>
        <div className="CommentInfoBox">
          <textarea
            ...
            className="CommentContents font-nanumgothic"
          />
        ...
        <div className="CommentFooter">
          <div className="CommentWrite font-jua" onClick={() => post_Comment()}>
            작성하기
          </div>
        </div>
      </div>
      <div className="NoticeComment">
        {totalCommentCnt ? (
          <div className="CommentCnt">댓글 {totalCommentCnt}</div>
        ) : (
          <></>
        )}
        {comments?.map((com) => (
        ...
        {totalCommentPage > commentPage ? (
          <div ref={setTarget}>{isLoadedComment && <p>Loading...</p>}</div>
        ) : (
          <></>
        )}
      </div>
